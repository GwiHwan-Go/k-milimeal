...
margin-left: 2px;
`;
export default function NotificationFlatListItems({item, index}){

        return(
        <Container>
          <Header>
            <UserAvatar resizeMode="cover" source={{ uri: item.avatar }} />
            <Username>{item.name}</Username>
            <View style={{flex:1, flexDirection:"row"}}>
            {item.foodArray.map((food)=>{
              return(
            ...
            <View style ={{flex:1,marginRight: 30,justifyContent: 'center', alignItems: 'center'}}>
              <Image source={{uri:item.imageUrl ,width : 610 , height: 300}}></Image>
            </View>
            <ExtraContainer>
              <Actions>
                    <CaptionText>{item.foodDescription}</CaptionText>
                    <Action >
